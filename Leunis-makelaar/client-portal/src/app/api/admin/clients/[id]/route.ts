...
export async function DELETE(
  ...
  if (offerteIds.length > 0) {
    ...
    if (sprintIdsError) return NextResponse.json({ error: sprintIdsError.message }, { status: 500 })
  ...
  if (offerteIds.length > 0) {
    ...
    if (onboardingQuestionIdsError) return NextResponse.json({ error: onboardingQuestionIdsError.message }, { status: 500 })
