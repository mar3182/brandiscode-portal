...
9. **Skills-based workflow** — gebruik `CONTEXT.md` en de lokale skills in `.github/skills/` voor alignering, verticale slices en kwaliteitscontrole. Start met een korte grill-session bij ambiguïteit en blijf klein.
10. **Deep modules** — ontwerp modules rond één verantwoordelijkheid met een kleine interface; vermijd dikke helpers of te veel onnodige abstracties.
11. **Feedback loops** — voor gedrag- of flow-wijzigingen: eerst een kleine repro/verwachting, dan implementatie, dan validatie via TypeScript/build.
